...
growth <- ln_cells %>% 
  mutate(exp_start = ifelse(Bottle == "A", 2, NA),
         exp_start = ifelse(Bottle == "B", 1, exp_start),
         exp_start = ifelse(Bottle %in% c("C", "D", "E", "F"), 1, exp_start),
         exp_start = ifelse(Bottle %in% c("G", "H"), 2, exp_start),
         exp_end = ifelse(Bottle %in% c("B", "C"), 3, 1),
        exp_end = ifelse(Bottle == "A", 4, exp_end)) %>% 
  group_by(Treatment, Bottle) %>% 
  mutate(ln_cells_exp_start = ifelse(Timepoint == exp_start, ln_cells, exp_start),
         ln_cells_exp_end = ifelse(Timepoint == exp_end, ln_cells, exp_end),
         cells_exp_start = ifelse(Timepoint == exp_start, cells, exp_start),
         cells_exp_end = ifelse(Timepoint == exp_end, cells, exp_end),
         days_exp_start = ifelse(Timepoint == exp_start, days, exp_start),
         days_exp_end = ifelse(Timepoint == exp_end, days, exp_end),) %>% 
...
check <- growth %>% 
  select(Experiment, Treatment, Bottle, exp_start, exp_end, ln_cells_exp_start:days_exp_end, mu:delta_cells) +
...
```{r}
lag + delta_bc + mu + doubling + plot_annotation(tag_levels = "a")
```
